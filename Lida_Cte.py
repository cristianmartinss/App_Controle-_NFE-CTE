...
def lerArquivo_CTE():
    ...
    layout = [
        [sg.Text('Selecione o arquivo com o CTE',background_color="#000000",pad=(110,5),text_color="#eeeeee")],
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            file_path = values["-FILE-"]
            if file_path:
                df = pd.read_excel(file_path, converters={'Chave de Acesso                              ':str,
    'Documento Emitente':str,'UF Emitente':str,'Documento Destinatário':str})
                df["Sistema"] = ""
                df["Emitente"] = ""
                window.close() 
                return df
